...
def get_corners_fast(image):
    ...
    A_11 = signal.convolve2d(I_xx, w, mode='same')
    A_12 = signal.convolve2d(I_xy, w, mode='same')
    A_22 = signal.convolve2d(I_yy, w, mode='same')
...
def compare(features_1, features_2):
    def fit_score(delta):
        dx, dy = delta
    ...
    res = minimize(fit_score, [0, 0], bounds=bnds, method='SLSQP')
